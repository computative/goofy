...
#lambda = 1e-7
   println("k = " , k)
   lambdas = 10.0 .^ Vector(-30.0:3:20.0)
   #lambdas = 10.0 .^ Vector(-20:-20)
   selector = 2
   fig_data = []
   for lambda in lambdas 
      ...
      c, _, e, basis, configs = train(system, ace_param, fit_param)   

      rmse = test(c, basis, configs, H[1:stride:end])
      append!(fig_data,[rmse[selector]])
   end
   plot!( lambdas, Float64.(fig_data), xscale=:log10, yscale=:log10 )
end
p 
