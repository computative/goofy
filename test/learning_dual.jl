...
for len in lens
    ...
    for M in 1:2 # M mode 
        ...
        x = [ norm(_R[M][i][_IJ[M][i][2],:] - _R[M][i][_IJ[M][i][1],:]) for i in 1:size(_R[M])[1] ]
        _y1 = [  Hpredict[i] .- _H[M][i] for i in 1:length(_H[M]) ]
        ...
        _y2 = [ abs.( _H[M][i] ) for i in 1:size(_R[M])[1] ]
