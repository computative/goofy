...
# execute with julia --project=. test/learning.jl n m l u v w x (where n and m are the id of the training and testing data. 
# l is regularization parameter, u and v are the order and degree respectively
# w is the number of matrces to sample, and x is the target matrix.
...
target_matrix_id = ARGS[7] # which matrix index in file id 1 should be the target for the prediction?
...
for len in lens 
    ...
    println("Eexact ", len, " target_matrix ", target_matrix_id, " ", Eexact)
    println("Epredict ", len, " target_matrix ", target_matrix_id, " ", Epredict)
end
